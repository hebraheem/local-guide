...
export default async function HelperDetailPage({ params }: Props) {
  ...
  const helper = (await getUserById(id))?.data;
  ...
  const reviews = [
    {
      ...
      rating: 5,
      comment:
        "Sarah translated my employment contract perfectly. Very professional and quick turnaround!",
      date: "3 days ago",
    },
    {
      name: "Anna Mueller",
      rating: 5,
      comment:
        "Helped me with my visa application documents. Everything was approved on first try!",
      date: "1 week ago",
    },
    {
      name: "David Chen",
      rating: 4,
      comment:
        "Great translator, very patient with revisions. Highly recommend!",
      date: "2 weeks ago",
    },
  ];
      ...
      <main className="mx-auto max-w-4xl px-4 py-6 space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
          ...
          <div className="flex flex-wrap gap-2">
            {helper?.profile?.languages?.map((lang, index) => (
              <span
                key={index}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-2">
              <span className="text-2xl">⭐</span>
              {t("PROFILE_REVIEWS")} ({reviews.length})
            </h2>
          </div>
