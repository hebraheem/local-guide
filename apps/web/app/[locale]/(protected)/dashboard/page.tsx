...
const Dashboard = async () => {
  ...
  if (!location?.latitude || !location?.longitude) {
    // If a user location is not available, fetch all users instead of nearby users
    helpers = (await getAllUsers({ limit: 5, role }))?.data;
  } else {
    helpers = (await getNearByUsers(location.latitude, location.longitude, 20))
      ?.data;
  }
          ...
          <div className="grid gap-3">
            {helpers.map((helper) => (
              <HelperCard
                ...
                avatarSrc={helper.profile?.avatarUrl ?? DEFAULT_AVATAR_URL}
